...
- **`client` (за твоим основным роутером)** — _режим по умолчанию, твой случай:_ у тебя  
...
2. Открой **Balena Etcher** → _Flash from file_ → выбери образ → выбери SD-карту → **Flash!**
...
scp -r pirouter pi@raspberrypi.local:~/ (raspberrypi.local это IP в сети)
...
|---|---|
|OpenVPN|Загрузи свой `.ovpn` в `/etc/pirouter/openvpn/client.conf`, затем выбери _OpenVPN_. `scp client.ovpn pi@192.168.33.1:/etc/pirouter/openvpn/client.conf`|
